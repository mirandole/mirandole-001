...
def test_adzuna_credentials_are_read_from_environment(
    ...
    monkeypatch.setenv("MIRANDOLE_ADZUNA_RESULTS_PER_PAGE", "25")
    monkeypatch.setenv("MIRANDOLE_ADZUNA_MAX_RESULTS", "75")
    ...
    assert settings.adzuna_results_per_page == 25
    assert settings.adzuna_max_results == 75
...
def test_enabled_adzuna_requires_credentials(
    ...
    with pytest.raises(ConfigError, match="ADZUNA_APP_ID is required"):
        Settings.from_env()
